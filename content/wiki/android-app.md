---
title: "Android 软件"
slug: "android-app"
toc: true
tocBorder: true
---

> 本文内容：安卓手机的 APP 安装备忘录

# APP 列表

- [[Download Link]](https://f-droid.org/zh_Hans/packages/) F-Droid
- [[Download Link]](https://cn.uptodown.com/windows) Uptodown
- [[Download Link]](https://github.com/rosuH/EasyWatermark/blob/master/README_zh-CN.md) 简单水印
- [[Download Link]](https://localsend.org/zh-CN) LocalSend
- [[Download Link]](https://github.com/ekibun/Stitch) Stitch
- [[Download Link]](https://www.chunjizhang.com/) 纯记账
- [[Download Link]](https://github.com/helloworld1/FreeOTPPlus) FreeOTP+
- Google Authenticator
- Wear OS
- 白描证件照【收费】【已买，手机号登录】
- Clash

# 杂项

## 国行三星手机安装谷歌件套

1. 安装 Uptodown App Store

	首先在 [uptodown-android](https://uptodown-android.cn.uptodown.com/android) 下载 Uptodown App Store 的 apk 然后安装在手机上。

2. 安装 Google Mobile Services (GMS)

	在 Uptodown App Store 里面安装 Google Mobile Services (GMS)。

3. 安装 Google Play

	安装 Google Mobile Services (GMS) 之后, 继续在 Uptodown App Store 里面安装 Google Play。
