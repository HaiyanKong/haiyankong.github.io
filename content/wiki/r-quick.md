...
| 方法                      | 描述                                         | 优点                                  | 缺点                                   |
| ------------------------- | -------------------------------------------- | ------------------------------------- | -------------------------------------- |
| utils::packageVersion()   | 直接查看指定包的版本                         | 简单直接                              | 需要单独指定每个包                     |
| installed.packages()      | 查看所有已安装包的信息，可提取特定包的信息   | 可以一次性获取多个包的信息            | 需要使用矩阵索引来提取信息             |
| sessionInfo()             | 显示 R 会话的详细信息，包括已加载包的版本    | 一次性显示所有已加载包的信息和 R 版本 | 只显示已加载的包，不显示所有已安装的包 |
| RStudio "Packages" 选项卡 | 通过图形界面查看                             | 界面友好，易于操作                    | 只能在 RStudio 中使用                  |
